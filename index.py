...
═══════════════════════════════════
流程（单轨迹粒度，支持断点续跑）:
  1. 扫描 data/{dataset}/traj_XXXX.md
  2. 增量提取 meta：跳过已有且合格的 .md.meta，不合格的重提
  3. 增量向量化：已在 index.pkl 中的跳过，只补新增
  4. 合并写回 index.pkl

用法:
  python index.py --dataset swe_smith_dataset
  python index.py --all
  python index.py --all --concurrency 20
  python index.py --dataset swe_smith_dataset --reindex   # 强制全量重建向量
"""
...
from modules.llm_client import LLMClient, EmbedClient, create_embed_client, create_llm_client
...
def main():
    parser = argparse.ArgumentParser(description="T2S 轨迹索引构建（增量版）")
    parser.add_argument("--dataset", type=str, help="数据集目录名 (在 data/ 下)")
