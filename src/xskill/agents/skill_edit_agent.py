...
atom 证据，但写入用户可见产物时只保留自包含的结论与证据强度，不暴露内部 id。
...
1. <逐条编号。每条三件套——**规则本身 / 为什么（机制）/ 适用条件**。>
   ...
   > 不依赖外部材料即可理解；不要凭空猜，末尾同样标证据强度。>

正文及 ``scripts/``、``references/`` 等用户可见产物不得出现 ``atom_id`` / ``traj_id``，
也不得要求读者查找原始 atom / traj。它们是内部来源信息，只允许保留在
``metadata.source_atoms``、commit message 和系统日志中。
