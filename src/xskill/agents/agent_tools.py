...
# 本次 generate 里最近一次 new_skill_folder 建出的 slug。write_file / edit 的
# 裸相对路径（如 "SKILL.md"）会落到这个目录下——generate 的 skill_dir 是多
# skill 根，不带 slug 的相对路径写到根上必然是错的（曾把正文写偏、提交了
# placeholder 的 SKILL.md）。SkillEdit 的 skill_dir 就是单 skill 目录，不受影响。
_GENERATE_ACTIVE_SKILL: contextvars.ContextVar[str] = contextvars.ContextVar(
    "xskill_generate_active_skill",
)
...
def reset_generate_session() -> None:
    ...
    _GENERATE_ACTIVE_SKILL.set("")


def _generate_active_skill() -> str:
    try:
        return _GENERATE_ACTIVE_SKILL.get()
    except LookupError:
        return ""
...
def _resolve_skill_write_path(path: str) -> tuple[Path | None, str | None]:
    ...
    # generate 任务里 skill_dir 是多 skill 根：裸 "SKILL.md" / "scripts/x.sh"
    # 落到根上是写偏。有活动 skill（本次 new_skill_folder 建的）且路径首段
    # 不是根下已有目录时，落到活动 skill 目录里。
    active = _generate_active_skill()
    if (
        active
        and not raw.is_absolute()
        and raw.parts
        and raw.parts[0] != active
        and not (skill_dir / raw.parts[0]).is_dir()
    ):
        raw = Path(active) / raw
...
def edit_file(path: str, old_string: str, new_string: str) -> str:
    ...
    否则报错。old_string 要能在文件里唯一命中：匹配不到或匹配多处都会报错，
    这时把上下文多带几行使其唯一。文件还不存在就用 write_file。
...
def new_skill_folder(skill_name: str, description: str) -> str:
    ...
    def mutate():
        if target.exists():
            _GENERATE_ACTIVE_SKILL.set(slug)
