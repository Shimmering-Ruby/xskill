...
async def api_skill_candidates(name: str):
    ...
    from traj2skill.candidates import load_candidates
    sd = _skill_dir / name
    if not sd.is_dir():
...
def _exec_tool(name: str, args: dict, skill_dir: Path, traj_dir: Path, config: dict) -> str:
    ...
    if name == "search_skills":
        from traj2skill.skill_tools import search_skills as _search_skills
...
async def api_chat_archive(req: dict):
    ...
    Writes traj_<id>.md to trajectory/chat_app_demo/ with t2s header.
    ...
    lines = []
    # t2s header for watcher to parse
    if skill_name:
        lines.append(f"<!-- t2s:skill={skill_name} side={side or 'main'} sha={sha} -->")
...
async def api_unregister_dir(req: dict):
    ...
    from traj2skill.registry import unregister_dir
    path = req.get("path", "")
    if not path:
        raise HTTPException(status_code=400, detail="path is required")
...
async def api_trajectory_logs(filename: str, dir: str = ""):
    ...
    from traj2skill.registry import list_watch_dirs, get_connection
    import json as _json
...
async def api_list_trajectories():
    ...
    from traj2skill.registry import list_watch_dirs, get_connection, get_status_counts
    dirs = list_watch_dirs()
    all_trajs = []
    for d in dirs:
        conn = get_connection()
...
def create_app() -> FastAPI:
    ...
    async def _startup():
        ...
        try:
            ...
            logger.info(
                "traj2skill server ready  skill_dir=%s  traj_dir=%s  llm=%s  embed=%s",
