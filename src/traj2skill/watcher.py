...
logger = logging.getLogger("t2s.watcher")
...
class DirectoryWatcher:
    ...
    def start(self):
        if self._thread and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._loop, daemon=True, name="t2s-watcher")
        self._thread.start()
    ...
    def _do_meta(self, dir_path, fname):
        ...
        from traj2skill.index import _process_one_meta, validate_meta
    ...
    def _do_embed(self, dir_path, wd_id, filenames):
        ...
        from traj2skill.index import build_vector_index_incremental
