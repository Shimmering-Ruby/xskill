...
| **openclaw-linux** | `~/.openclaw/agents/<agent-id>/sessions/<sid>.trajectory.jsonl`<br/>同目录 `<sid>.trajectory-path.json`（指针）<br/>覆盖：`OPENCLAW_TRAJECTORY_DIR`；关停：`OPENCLAW_TRAJECTORY=false` | 1️⃣ `<workspace>/skills/`<br/>2️⃣ `<workspace>/.agents/skills/`<br/>3️⃣ `~/.agents/skills/`<br/>4️⃣ `~/.openclaw/skills/`<br/>5️⃣ `<install>/skills/`（bundled，53 个）<br/>6️⃣ `skills.load.extraDirs[]` + plugin skills（最低） | **Workspace > Project-agent > Personal-agent > Managed > Bundled > Extra**（高→低）<br/>`fs.realpathSync` 去重 | 系统提示词预注入 `<available_skills>` XML（含 `<location>`）；无 tool call<br/>token 公式：`195 + Σ(97 + 字段长度)` 字节 |
...
| **OpenClaw** | **`2026.5.7 (eeef486)`** | npm 全局：`~/.nvm/versions/node/v24.14.1/lib/node_modules/openclaw/` | ✅ `~/.openclaw/agents/main/sessions/*.trajectory.jsonl` |
...
| OpenClaw | JSONL trace events（每行带 `traceSchema/schemaVersion/seq/sourceSeq/ts/type/data`）；同目录 `.trajectory-path.json` 指针 | 同步入队 → 队列化文件写；单事件 ≤256 KB，单文件 ≤50 MB，超限 `trace.truncated` | ✅ 行级 tail；glob 必须匹配 `*.trajectory.jsonl` 排除运行时 `*.jsonl` |
...
| OpenClaw | **未支持**，设计见 [`openclaw.md`](./openclaw.md)；本机有数据 (`openclaw@2026.5.7`) | 拟新增 `install_to_openclaw` / `ingest_openclaw_sessions` / `_adapt_openclaw_trajectory_jsonl` |
