...
1. **P0** 确认 `create_temp` / `kernel-temp` 的 `auto_index` 是否已能被 watcher 拆分；否则 benchmark 链路空转。  
2. **P1** 与 OE 对齐：benchmark rollout 进 training 是否可接受；否则应仅作 Gate/审计。  
3. **P1** 多 atom + oracle 报错：改 OE 约束或平台「整轨单 atom」能力（此前倾向不在平台加 OE 专用模式）。  
4. **P2** 观察发布队列 latest-wins 是否够用；是否要平台级排队。  
5. **P2** 复测 0.9.0 wheel 行为与文档一致性（含 `full_rebuild` 才跑 benchmark）。
