...
- **2026-08-03** `v0.6.29a6`：`pymilvus` 改为可选依赖（`xskill[milvus]`），未安装时走 numpy/内存 fallback 并每小时告警，修复因硬依赖阻断 client 自动更新；「我的」页支持「推给我 N 个 SKILL」client 截取安装、上传 skill 使用去向与 skill commit 三态；SkillHub 精确名/ID 匹配置顶；看板展示推荐版本赋值与当前推送对象。
- **2026-08-03** `v0.6.29a5`：推荐重活进程与 Web 隔离，引入 Milvus Lite 向量索引对账与脏用户推荐预计算。
- **2026-07-30** `v0.6.29a4`：Dashboard 新增「我的」页，普通用户默认进入，贡献去向与世界消息支持分页浏览；安装账本支持存量孤儿安装自愈与清单外孤儿回收，修复 Windows 客户端元数据校验风暴导致的回流冻结。
- **2026-07-30** `v0.6.29a3`：推荐引擎改为纯相关度轮询并按兴趣中心分流，UX 评分持久化到 registry.db；安装账本 SQLite 化，卸装改为事务化 supersede。
